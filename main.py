...
def seed_inicial():
    with SessionLocal() as session:
        ...
        if not session.query(UsuarioDB).filter(
                UsuarioDB.rol == "supervisor").first():
        ...
        defaults_cfg = [
            ...
            ("dias_laborables", "5",
             "Días laborables por semana (1=Lun … 7=Dom)"),
            ("alertas_jornada_pct", "70",
             "% mínimo de jornada para no generar alerta"),
        ]
        for clave, valor, desc in defaults_cfg:
            if not session.query(ConfiguracionDB).filter(
                    ConfiguracionDB.clave == clave).first():
        ...
        for nombre, color in tareas_default:
            if not session.query(TareaDB).filter(
                    TareaDB.nombre == nombre).first():
                session.add(TareaDB(nombre=nombre, color=color))

        session.flush()

        # Subtareas por defecto
        subtareas_default = {
            "Picking":   ["Picking RF", "Picking Manual",
                          "Picking Lote", "Revisión de Picking"],
            "Packing":   ["Embalaje Estándar", "Embalaje Frágil",
                          "Etiquetado", "Precintado"],
            "Inbound":   ["Recepción de mercancía", "Desconsolidación",
                          "Control de calidad", "Ubicación en estantería"],
            "Shipping":  ["Carga de camión", "Validación de albaranes",
                          "Expedición urgente"],
            "Ecommerce": ["Preparación de pedidos B2C",
                          "Gestión de devoluciones",
                          "Embalaje especial E-commerce"],
        }
        for nombre_tarea, subs in subtareas_default.items():
            tarea = session.query(TareaDB).filter(
                TareaDB.nombre == nombre_tarea).first()
            if not tarea:
                continue
            for sub in subs:
                if not session.query(SubtareaDB).filter(
                        SubtareaDB.tarea_id == tarea.id,
                        SubtareaDB.nombre == sub).first():
                    session.add(SubtareaDB(
                        tarea_id=tarea.id, nombre=sub))

        # Objetivos por defecto
        for nombre_tarea, _ in tareas_default:
            if not session.query(ObjetivoDB).filter(
                    ObjetivoDB.tarea_principal == nombre_tarea).first():
                session.add(ObjetivoDB(
                    tarea_principal=nombre_tarea,
                    uds_hora=100.0,
                    horas_jornada=8.0
                ))
...
def get_config_valor(db: Session, clave: str,
                     default: str = "480") -> str:
    cfg = session.query(ConfiguracionDB).filter(
        ConfiguracionDB.clave == clave).first() \
        if False else \
        db.query(ConfiguracionDB).filter(
            ConfiguracionDB.clave == clave).first()
    return cfg.valor if cfg else default

def get_minutos_jornada(db: Session) -> int:
    return int(get_config_valor(db, "minutos_jornada", "480"))
...
class ConfiguracionRespuesta(ConfiguracionCreate):
    id: int
    class Config:
        from_attributes = True
...
def login(datos: LoginRequest, db: Session = Depends(get_db)):
    usuario = db.query(UsuarioDB).filter(
        ...
        UsuarioDB.pin    == hash_pin(datos.pin)
    ).first()
    if not usuario:
        raise HTTPException(
            status_code=401,
...
def crear_usuario(usuario: UsuarioCreate, db: Session = Depends(get_db)):
    if db.query(UsuarioDB).filter(
            UsuarioDB.codigo == usuario.codigo).first():
        raise HTTPException(
            status_code=400,
...
@app.put("/usuarios/{id}/pin", status_code=200)
def cambiar_pin(id: int, datos: LoginRequest,
...
def obtener_tareas(db: Session = Depends(get_db)):
    return db.query(TareaDB).order_by(TareaDB.nombre).all()
...
def crear_tarea(tarea: TareaCreate, db: Session = Depends(get_db)):
    if db.query(TareaDB).filter(
            TareaDB.nombre == tarea.nombre).first():
        raise HTTPException(
            status_code=400,
...
def eliminar_tarea(id: int, db: Session = Depends(get_db)):
    ...
    db.query(SubtareaDB).filter(SubtareaDB.tarea_id == id).delete()
    db.delete(tarea)
    db.commit()
...
def eliminar_subtarea(id: int, db: Session = Depends(get_db)):
    sub = db.query(SubtareaDB).filter(SubtareaDB.id == id).first()
    if not sub:
        raise HTTPException(
            status_code=404, detail="Subtarea no encontrada")
    db.delete(sub)
    db.commit()
...
def obtener_configuracion(db: Session = Depends(get_db)):
    return db.query(ConfiguracionDB).order_by(
        ConfiguracionDB.clave).all()

@app.put("/configuracion/{clave}",
         response_model=ConfiguracionRespuesta)
def actualizar_configuracion(clave: str,
                              cfg: ConfiguracionCreate,
                              db: Session = Depends(get_db)):
    item = db.query(ConfiguracionDB).filter(
        ConfiguracionDB.clave == clave).first()
    if not item:
        item = ConfiguracionDB(
            clave=cfg.clave,
            valor=cfg.valor,
            descripcion=cfg.descripcion)
        db.add(item)
    else:
        item.valor       = cfg.valor
        item.descripcion = cfg.descripcion or item.descripcion
    db.commit()
    db.refresh(item)
    return item
# ==================== REGISTROS ====================
...
@app.post("/registros/",
          response_model=RegistroTareaRespuesta, status_code=201)
def crear_registro(registro: RegistroTarea,
                   db: Session = Depends(get_db)):
    ...
    total_min = sum(r.tiempo_minutos for r in total)
    if total_min + registro.tiempo_minutos > minutos_jornada:
        raise HTTPException(
            status_code=400,
            detail=f"No se pueden superar {minutos_jornada} minutos. "
                   f"Minutos ya registrados: {total_min}")
...
def obtener_registros(db: Session = Depends(get_db)):
    return db.query(RegistroDB).order_by(
        RegistroDB.fecha.desc()).all()

@app.get("/registros/rango/",
...
def resumen_mensual(
    ...
    fin    = (date(anyo + 1, 1, 1) - timedelta(days=1)
              if mes == 12
...
def editar_registro(id: int, registro: RegistroTarea,
                    db: Session = Depends(get_db)):
    minutos_jornada = get_minutos_jornada(db)
    db_reg = db.query(RegistroDB).filter(
        RegistroDB.id == id).first()
    if not db_reg:
        raise HTTPException(
...
@app.post("/registros/{id}/duplicar",
          response_model=RegistroTareaRespuesta, status_code=201)
def duplicar_registro(id: int,
    ...
    total_min = sum(r.tiempo_minutos for r in total)
    if total_min + original.tiempo_minutos > minutos_jornada:
        raise HTTPException(
            status_code=400,
            detail=f"No se pueden superar {minutos_jornada} minutos. "
                   f"Minutos ya registrados: {total_min}")
...
def crear_plantilla(plantilla: PlantillaCreate,
                    db: Session = Depends(get_db)):
    nueva = PlantillaDB(
...
@app.get("/plantillas/")
def obtener_plantillas(usuario: Optional[str] = None,
                       db: Session = Depends(get_db)):
    ...
    for p in query.all():
        items = db.query(PlantillaItemDB).filter(
            PlantillaItemDB.plantilla_id == p.id).all()
        resultado.append({
            "id":         p.id,
            "nombre":     p.nombre,
            "usuario":    p.usuario,
...
def actualizar_objetivo(tarea: str,
                        objetivo: ObjetivoCreate,
                        db: Session = Depends(get_db)):
    db_obj = db.query(ObjetivoDB).filter(
        ObjetivoDB.tarea_principal == tarea).first()
    if not db_obj:
        db_obj = ObjetivoDB(
...
@app.post("/volumenes/",
          response_model=VolumenRespuesta, status_code=201)
def crear_volumen(volumen: VolumenCreate,
                  db: Session = Depends(get_db)):
...
def editar_volumen(id: int, volumen: VolumenCreate,
                   db: Session = Depends(get_db)):
    db_vol = db.query(VolumenDB).filter(
        VolumenDB.id == id).first()
    if not db_vol:
        raise HTTPException(
            status_code=404, detail="Volumen no encontrado")
...
def obtener_metricas(
    ...
    volumenes  = db.query(VolumenDB).filter(
        ...
        VolumenDB.fecha <= hasta).all()
    registros  = db.query(RegistroDB).filter(
        RegistroDB.fecha >= desde,
        RegistroDB.fecha <= hasta).all()
    resultado  = []
    for v in volumenes:
        horas_reales = sum(
        ...
        uds_hora_real = round(v.unidades / horas_reales, 1) \
                        if horas_reales > 0 else None
        uds_hora_teo  = round(v.unidades / v.horas_teoricas, 1) \
...
@app.post("/unidades/",
          response_model=UnidadRespuesta, status_code=201)
def crear_unidad(unidad: UnidadCreate,
                 db: Session = Depends(get_db)):
    nueva = UnidadDB(**unidad.dict())
    db.add(nueva)
    db.commit()
    db.refresh(nueva)
    return nueva

@app.get("/unidades/", response_model=List[UnidadRespuesta])
...
@app.delete("/unidades/{id}", status_code=204)
def eliminar_unidad(id: int, db: Session = Depends(get_db)):
    u = db.query(UnidadDB).filter(UnidadDB.id == id).first()
    if not u:
        raise HTTPException(
            status_code=404, detail="Registro de unidades no encontrado")
    db.delete(u)
    db.commit()

@app.post("/unidades/importar/")
async def importar_unidades(
    ...
    for i, fila in enumerate(filas, start=2):
        try:
            ...
            tarea      = str(fila[2]).strip()
            unidades   = int(fila[3])
            comentarios = str(fila[4]).strip() if len(fila) > 4 \
                          and fila[4] else None
            ...
            if isinstance(fecha_raw, datetime):
                ...
                fecha = date.fromisoformat(str(fecha_raw).strip())
...
def rendimiento_unidades(
    ...
    if not hasta:
        hasta = date.today()
    unidades  = db.query(UnidadDB).filter(
    ...
    registros = db.query(RegistroDB).filter(
        ...
        RegistroDB.fecha <= hasta)
    if usuario:
        unidades  = unidades.filter(UnidadDB.usuario == usuario)
        registros = registros.filter(RegistroDB.usuario == usuario)
    ...
    for u in unidades:
        horas_reales = sum(
            r.tiempo_minutos for r in registros
            if str(r.fecha) == str(u.fecha)
            and r.usuario == u.usuario
            and r.tarea_principal == u.tarea_principal
        ) / 60
        obj          = objetivos.get(u.tarea_principal)
        uds_hora_obj = obj.uds_hora if obj else None
        uds_hora_real = round(u.unidades / horas_reales, 2) \
                        if horas_reales > 0 else None
        pct_objetivo  = round(
        ...
        resultado.append({
            "fecha":           str(u.fecha),
            "usuario":         u.usuario,
            "tarea_principal": u.tarea_principal,
            "unidades":        u.unidades,
            "horas_reales":    round(horas_reales, 2),
            "uds_hora_real":   uds_hora_real,
            "uds_hora_objetivo": uds_hora_obj,
            "pct_objetivo":    pct_objetivo,
...
def plantilla_excel_unidades():
    ...
    ws = wb.active
    ws.title = "Importar Unidades"
    cabeceras = [
        "fecha (YYYY-MM-DD)", "usuario", "tarea_principal",
    ...
    # Fila de ejemplo
    ws.append([
        str(date.today()), "EMP001", "Picking", 250, "Turno mañana"
    ])
...
def top_tareas(
    ...
    registros = db.query(RegistroDB).filter(
        ...
        RegistroDB.fecha <= fecha_hasta).all()
    por_tarea = {}
    for r in registros:
...
def ranking_operarios(
    ...
    for u in usuarios:
        ...
        pct_jornada = round(
            (total_mins / max_mins) * 100, 1) if max_mins > 0 else 0
        consistencia = round(
            (dias_activo / dias_laborables) * 100, 1) \
            if dias_laborables > 0 else 0
        media_diaria = round(
            total_mins / dias_activo, 0) if dias_activo > 0 else 0
        resultado.append({
            "usuario":             u.codigo,
            "nombre":              u.nombre,
...
def alertas_rendimiento(db: Session = Depends(get_db)):
    ...
    minutos_jornada  = get_minutos_jornada(db)
    pct_minimo       = int(get_config_valor(
        db, "alertas_jornada_pct", "70"))
    usuarios         = db.query(UsuarioDB).filter(
        UsuarioDB.rol == "operario").all()
    registros_hoy    = db.query(RegistroDB).filter(
        RegistroDB.fecha == hoy).all()
    ...
    for u in usuarios:
        ...
        if mins_hoy > 0 and pct_hoy < pct_minimo:
            alertas.append({
                "tipo":    "jornada_incompleta",
                "nivel":   "warning",
                "usuario": u.codigo,
                "nombre":  u.nombre,
                "mensaje": f"{u.nombre} lleva solo {pct_hoy}% "
        ...
        if 0 < dias_activo_semana < 3:
            alertas.append({
                ...
                "usuario": u.codigo,
                "nombre":  u.nombre,
                "mensaje": f"{u.nombre} solo ha registrado "
                           f"{dias_activo_semana} días esta semana"
            })

    volumenes = db.query(VolumenDB).filter(
        VolumenDB.fecha >= hace_7,
        VolumenDB.fecha <= hoy).all()
    for v in volumenes:
        horas_reales = sum(
            r.tiempo_minutos for r in registros_semana
            if str(r.fecha) == str(v.fecha)
        ...
        if horas_reales > 0:
            ...
            if eficiencia < 70:
                alertas.append({
                    ...
                    "mensaje": f"Eficiencia baja en {v.tarea_principal} "
                               f"el {v.fecha}: {eficiencia}%"
                })
            elif abs(horas_reales - v.horas_teoricas) \
                ...
                alertas.append({
                    ...
                    "usuario": None,
                    "nombre":  v.tarea_principal,
                    "mensaje": f"Desviación >20% en {v.tarea_principal} "
                               f"el {v.fecha}: "
...
def exportar_excel(
    ...
    def autoajustar(ws):
        for col in ws.columns:
            ...
            ws.column_dimensions[
                col[0].column_letter].width = min(max_length + 4, 40)
    ...
    ws0.append(["Media horas/día",
                round(total_mins / 60 / dias_unicos, 2)
    ...
    for (tp, st), d in sorted(por_tarea.items()):
        media = round(d["minutos"] / d["registros"], 1) \
                if d["registros"] > 0 else 0
        ws3.append([
            tp, st, d["registros"],
            d["minutos"], round(d["minutos"] / 60, 2), media
        ])
    autoajustar(ws3)
    ...
    ws4.append([
        ...
        "Usuarios Activos", "% Jornada Media"
    ])
    estilo_cabecera(ws4)
    dias_semana = [
    ...
    for r in registros:
        ...
        if dia not in por_dia:
            por_dia[dia] = {
                "registros": 0, "minutos": 0, "usuarios": set()}
    ...
    for dia, d in sorted(por_dia.items(), reverse=True):
        ...
        pct        = round(
            (d["minutos"] / (n_usuarios * minutos_jornada)) * 100, 1) \
            if n_usuarios else 0
        ws4.append([
            dia, nombre_dia, d["registros"],
            d["minutos"], round(d["minutos"] / 60, 2),
            n_usuarios, pct
        ])
    autoajustar(ws4)
    ...
    ws5.append([
        "Fecha", "Tarea", "Unidades", "Horas Teóricas",
        "Horas Reales", "Eficiencia %",
        "Uds/Hora Real", "Uds/Hora Teórica",
        "Desviación (h)", "Comentarios"
    ])
    estilo_cabecera(ws5)
    volumenes      = db.query(VolumenDB).order_by(
        VolumenDB.fecha.desc()).all()
    ...
    for v in volumenes:
        ...
        eficiencia = round(
            (v.horas_teoricas / horas_reales) * 100, 1) \
            if horas_reales > 0 else 0
        uds_real = round(v.unidades / horas_reales, 1) \
                   if horas_reales > 0 else 0
        uds_teo  = round(v.unidades / v.horas_teoricas, 1) \
                   if v.horas_teoricas > 0 else 0
        desviacion = round(horas_reales - v.horas_teoricas, 2)
        ws5.append([
            str(v.fecha), v.tarea_principal, v.unidades,
            v.horas_teoricas, round(horas_reales, 2),
            eficiencia, uds_real, uds_teo,
    ...
    ws6.append([
        "Posición", "Usuario", "Total Horas",
        "Días Activos", "% Jornada Periodo",
        "Consistencia %", "Media Min/Día"
    ])
    estilo_cabecera(ws6)
    ranking = sorted(
        por_usuario.items(),
    ...
    for pos, (u, d) in enumerate(ranking, 1):
        ...
        pct_per   = round(
            (d["minutos"] / (dias * minutos_jornada)) * 100, 1)
        ws6.append([
            pos, u,
            round(d["minutos"] / 60, 1),
            len(d["dias"]), pct_per,
            round((dias / max(dias_unicos, 1)) * 100, 1),
            media
        ])
    autoajustar(ws6)
