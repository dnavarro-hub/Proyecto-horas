...
class UsuarioDB(Base):
    ...
    pin     = Column(String)
    rol     = Column(String, default="operario", index=True)
...
class ObjetivoDB(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            'tarea_principal', 'subtarea',
            name='uq_objetivo_tarea_subtarea'
        ),
    )
    id              = Column(Integer, primary_key=True, index=True)
    tarea_principal = Column(String, index=True)
...
class ProductividadExcelDB(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            'fecha', 'usuario', 'tarea_principal', 'subtarea',
            name='uq_productividad_excel'
        ),
    )
    id              = Column(Integer, primary_key=True, index=True)
    fecha           = Column(Date, index=True)
...
SUBTAREAS_DEFAULT = {
    "Picking":   ["Picking Balda","Picking Palletl","Picking Percha",
                  "Revisión de Picking","Picking Kardex","Picking Recogepedidos",
                  "Picking Obsoleto","Inventario","Reposiciones","Traspasos",
                  "Recepción Kardex","Formacion","Compactar","Incidencias",
                  "Varios IT","Varios Trigo","Lanzar pedidos","Reuniones","Otros"],
    "Packing":   ["Mecado Piscina","Mercado Contenedor","Tienda Etiquetado",
                  "Wholesale","Tienda RFID","Tienda Empleado y Otros","Materiales",
                  "Runner","Formacion","incidencias","Reuniones","Otros","Admin"],
    ...
    "Ecommerce": ["Empaquetado","Runner","Store RQ Tienda","Calidad Devo",
                  "Calidad OneStock","Calidad Gestion","Calidad Otro","Formacion",
                  "Actividad","Limpieza","Otros","Reuniones"],
}
...
class VolumenCreate(BaseModel):
    ...
    @field_validator("horas_teoricas", mode="before")
    @classmethod
...
class ObjetivoCreate(BaseModel):
    ...
    @field_validator("uds_hora", mode="before")
    @classmethod
...
def crear_tarea(tarea: TareaCreate, db: Session = Depends(get_db)):
    ...
    nueva = TareaDB(**tarea.model_dump())
    db.add(nueva)
    db.commit()
    db.refresh(nueva)
    return nueva
...
def editar_tarea(id: int, tarea: TareaCreate, db: Session = Depends(get_db)):
    ...
    for k, v in tarea.model_dump().items():
...
def crear_subtarea(subtarea: SubtareaCreate, db: Session = Depends(get_db)):
    if db.query(SubtareaDB).filter(
        ...
        raise HTTPException(
            status_code=400,
            detail="Ya existe esa subtarea para esta tarea"
        )
    nueva = SubtareaDB(**subtarea.model_dump())
    db.add(nueva)
    db.commit()
    db.refresh(nueva)
    return nueva
...
def editar_subtarea(
    ...
    for k, v in subtarea.model_dump().items():
        setattr(db_sub, k, v)
    db.commit()
    db.refresh(db_sub)
    return db_sub
...
def actualizar_configuracion(
    ...
    return config
# ==================== REGISTROS ====================
# IMPORTANTE: rutas específicas SIEMPRE antes de /{usuario}
...
def crear_registro(registro: RegistroTarea, db: Session = Depends(get_db)):
    ...
    nuevo = RegistroDB(**registro.model_dump())
    db.add(nuevo)
    db.commit()
    db.refresh(nuevo)
    return nuevo
...
# ---- Rutas específicas ANTES de /{usuario} ----
...
def editar_registro(
    ...
    for k, v in registro.model_dump().items():
        setattr(db_reg, k, v)
    db_reg.updated_at = now_utc()
    db.commit()
    db.refresh(db_reg)
    return db_reg
...
def crear_volumen(volumen: VolumenCreate, db: Session = Depends(get_db)):
    nuevo = VolumenDB(**volumen.model_dump())
    db.add(nuevo)
    db.commit()
    db.refresh(nuevo)
    return nuevo
...
def editar_volumen(
    ...
    for k, v in volumen.model_dump().items():
        setattr(db_vol, k, v)
    db_vol.updated_at = now_utc()
    db.commit()
    db.refresh(db_vol)
    return db_vol
...
def obtener_metricas(
    ...
    for v in volumenes:
        ...
        uds_hora_real    = round(v.unidades / horas_reales, 1) \
                           if horas_reales > 0 else None
        uds_hora_teorica = round(v.unidades / v.horas_teoricas, 1) \
...
async def subir_excel_productividad(
    ...
    if not file.filename.endswith((".xlsx", ".xls")):
        raise HTTPException(
            status_code=400,
            detail="El fichero debe ser .xlsx o .xls"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="No se pudo leer el fichero Excel"
        )
    ...
    if not columnas_requeridas.issubset(set(df.columns)):
        raise HTTPException(
            status_code=400,
            detail=f"El Excel debe tener las columnas: "
    ...
    for _, fila in df.iterrows():
        try:
            ...
            if not usuario_db:
                errores.append(
                    f"Operario no encontrado: {fila['operario']}"
                )
                continue
...
def calcular_productividad(
    ...
    if usuario:
        query_excel = query_excel.filter(
            ProductividadExcelDB.usuario == usuario
        )
    if tarea:
        query_excel = query_excel.filter(
            ProductividadExcelDB.tarea_principal == tarea
        )
    if subtarea:
        query_excel = query_excel.filter(
            ProductividadExcelDB.subtarea == subtarea
        )
    ...
    for ex in registros_excel:
        ...
        uds_hora_real = round(ex.unidades / horas_reales, 1) \
                        if horas_reales > 0 else None
    ...
    campos_validos = {
        "uds_hora_real", "eficiencia_pct",
        "unidades", "horas_reales", "fecha"
    }
...
def resumen_productividad(
    ...
    detalle = calcular_productividad(desde=desde, hasta=hasta, db=db)
    resumen = {}
    for d in detalle:
        u = d["usuario"]
        if u not in resumen:
    ...
    for u, datos in resumen.items():
        uds_hora_media = (
            round(
                sum(datos["uds_hora_values"]) / len(datos["uds_hora_values"]), 1
            )
            if datos["uds_hora_values"] else None
        )
        eficiencia_media = (
            round(
                sum(datos["eficiencias"]) / len(datos["eficiencias"]), 1
            )
    ...
    for u in usuarios:
        ...
        resultado.append({
            ...
            "porcentaje_jornada": round(
                (total_hoy / minutos_jornada) * 100, 1
            )
        })
    return resultado
    ...
    for u in usuarios:
        ...
        if mins_hoy > 0 and pct_hoy < 70:
            alertas.append({
                ...
                "usuario" : u.codigo,
                "nombre"  : u.nombre,
                "mensaje" : f"{u.nombre} lleva solo {pct_hoy}% de la jornada "
                            f"({mins_hoy} min)"
            })
...
def objetivo_vs_real(
    ...
    for tarea, datos in por_tarea.items():
        ...
        pct = round(
            (horas_reales / horas_objetivo) * 100, 1
        ) if horas_objetivo else None
        resultado.append({
            "tarea"            : tarea,
            "horas_reales"     : horas_reales,
...
def exportar_excel(
    ...
    ws5.append([
        ...
        "Horas Reales", "Eficiencia %", "Uds/Hora Real",
        "Uds/Hora Teórica", "Desviación (h)", "Comentarios"
    ])
    estilo_cabecera(ws5)
    ...
    for v in volumenes:
        ...
        uds_real = round(
            v.unidades / horas_reales, 1
        ) if horas_reales > 0 else 0
        uds_teo = round(
            v.unidades / v.horas_teoricas, 1
    ...
    for p in prod_data:
        ws6.append([
            ...
            p["unidades"],
            p["horas_reales"],
            p["uds_hora_real"]    if p["uds_hora_real"]    is not None else "—",
            p["uds_hora_objetivo"] if p["uds_hora_objetivo"] is not None else "—",
            p["eficiencia_pct"]   if p["eficiencia_pct"]   is not None else "—"
        ])
    autoajustar(ws6)
